...
def _closest_to_date(observations: list[dict], target: date) -> float | None:
    """Return the value from the observation whose date is closest to target."""
    best = None
    best_delta = None
...
class YieldCurveRepo:
    def get(self, config: dict) -> dict:
        try:
            ...
            inverted = current < config["fred"]["series"]["T10Y2Y"]["inversion_threshold"]
            trending_negative = (prior is not None) and (current < prior)
            flagged = inverted and trending_negative

            return {
                "current": current,
                "prior": prior,
                "flagged": flagged,
                "label": config["fred"]["series"]["T10Y2Y"]["label"],
            }
